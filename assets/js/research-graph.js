...
(function () {
  ...
  var N = {
    ...
    exposure: {
      ...
      plain: 'At least six serious measures score how exposed each occupation is to AI, and a study that reports a share of jobs at risk picks one of them without saying why. If the measures disagree, every ranking of at-risk occupations carries the instrument as well as the labor market. The usual comparison, a rank correlation, weights an occupation of five hundred people the same as one of three million and is settled by the tails, where every measure agrees. This note compares the measures on the dollars they classify as exposed, at a common cut and decile by decile, with everything but the measure held fixed, and writes down before the computation what each answer would mean.',
    ...
    shortage: {
      ...
      plain: 'Headline shortage figures subtract the supply of one occupation from the demand for that same occupation, as though every unfilled position had to be filled by somebody newly trained for exactly it. The occupational flow account shows that in every occupation it covers, arrivals from another occupation exceed arrivals from education, by a median factor of 19. For five healthcare occupations in North Carolina, arrivals from other occupations at the observed rate come to between 49 and 158 per cent of the openings the state projection expects each year, so the channel the arithmetic leaves out is of the same size as the openings themselves. The paper writes the correction down, tier by tier of skill adjacency with every term measured and capped by history, and reads licensing off the pairs nobody crosses rather than entering it as a parameter.',
