...
(function () {
  ...
  var N = {
    ...
    sagetool: {
      ...
      plain: 'The paper argues that a household’s social fabric belongs inside the model rather than outside it, and the explorer is where anyone can check whether that argument does any work, by turning the social terms up and down and seeing which results move and which stay where they were. It is the difference between asking a reader to accept a claim about a model and handing them the model, and it is also the reason the thesis has had a life beyond its own publication.',
    ...
    skillsdna: {
      ...
      plain: 'Think of every industry as a cookbook and its output as the finished meals, where a role is a chef assigned a bundle of recipe steps, a task is a single one of those steps, and a skill is what lets a person perform it. Government data already records how many chefs of each kind there are and what they are paid, which steps each performs, and which abilities each step demands, so pushing the pay down through those links creates and destroys nothing and the same national total simply appears four times over, as industry output, as occupational pay, as the value of tasks, and as the value of skills. AI then enters as an appliance that can perform some of the priced steps, which turns exposure from an index number into a flow of money you can point at.',
    ...
    moves: {
      ...
      abs: 'Skill-similarity scores derived from occupational skill, knowledge and activity profiles are widely used as though a high score were evidence of realized mobility, when the score is a hypothesis about feasibility. This paper estimates a gravity model of observed occupational flows on public microdata and reads off the elasticity of moves to skill similarity, holding occupation size, wage differentials and geography constant, with licensing at the destination entered directly. It then tests whether the similarity thresholds the field uses to define transition tiers correspond to where observed flows actually break. Finding that flows break somewhere other than the similarity tiers would overturn the tiering convention, which is half the reason for asking.',
    ...
    shortage: {
      ...
      abs: 'Headline occupational shortage estimates subtract the supply of an occupation from the demand for that same occupation, which implicitly assumes zero substitution. Because demand is for output rather than for credentials, employers substitute across near occupations, and ignoring that margin overstates shortages materially. This paper recomputes the gap by subtracting, tier by tier of skill adjacency, the workers who can actually fill it, weighting each tier by observed transition rates and then re-restricting by licensing barriers at the destination. The correction deflates headline gaps by 15 to 20 percent. The approach is deliberately arithmetic rather than equilibrium modelling, so that any workforce board can apply and audit it. Worked cases in which the substitution margin absorbs nothing once licensing is graded in would overturn the result.',
