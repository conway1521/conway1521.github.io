...
(function () {
  ...
  var N = {
    ...
    p2: {
      ...
      plain: 'When nothing is disturbing it, a tightly connected system can look like a single dimension, and its separate parts show only under stress. In calm years the three pillars of the framework move together, and one factor fits the regional indicators better than three. When the pandemic hits, the link between the people side and the economic side breaks while the two links involving place barely move. A connected system would produce that pattern and independent dimensions would not. The regions sort into four tiers, which differ a lot in how well they hold their members. The struggling tier kept more than nine in ten of its regions through the shock, while the high-performing tier lost close to half, so regions at the bottom tend to stay there and regions at the top often slip.',
    ...
    exposure: {
      ...
      plain: 'At least six serious measures score how exposed each occupation is to AI, and studies that report a share of jobs at risk usually pick one without saying why. If the measures disagree, any ranking of at-risk occupations reflects the choice of measure as well as the labor market. The usual comparison is a rank correlation, which weights an occupation of five hundred people the same as one of three million and is driven by the extremes, where all the measures agree. This note compares the measures by the wage dollars they classify as exposed, at a common cutoff and decile by decile, holding everything else fixed, and states in advance what each result would mean.',
      abs: 'At least six measures score occupational exposure to artificial intelligence and studies select one without justification. The companion paper placed each measure at the level where it was collected and found the capability measures closer to one construct than their agreement suggests, the Srinivasan automation score being the Eloundou construction re-run and its augmentation score a transform of automation, and it showed that testing capability against observed usage selects on the outcome. This note fixes the comparison policy needs: the share of the wage bill on which two measures disagree about whether an occupation is exposed at a common cut, over 829 occupations carrying 10.70 trillion dollars, together with the agreement rate by decile of the pooled ranking, which separates the tails from the interior where targeting decisions are made. The design holds the occupational base, the vintages and every downstream computation fixed and varies only the measure, which the engine\u2019s adapter contract makes possible. Two downstream tests follow: the short-run association of each measure with realised 2022 to 2025 wage and employment changes under placebo-year inference, and one adoption scenario pushed through the engine under each measure. The statistic is implemented and verified, the readings are fixed, and the computation follows.',
    ...
    moves: {
      ...
      plain: 'A whole industry now advises workers and workforce boards with similarity scores, telling a machinist, for example, that industrial maintenance is 98.5 percent similar and only a few months of retraining away. A high score says a move is possible, but it does not show that workers make it. If the scores are wrong, the advice sends people toward jobs they will not get and public money after them. The data to check this is public, and the check is cheap. The paper sets up the test as a gravity model on the occupation changes that the CPS March supplement records within a single interview, the only design in that survey that separates real moves from coding noise. Similarity explains how much traffic flows between two occupations and the wage difference explains its direction, and the field’s thresholds at 0.985, 0.970 and 0.950 are compared with where the flows drop off.',
      abs: 'Skill-similarity scores derived from occupational skill, knowledge and activity profiles are used as though a high score were evidence of realised mobility, when the score is a hypothesis about feasibility. This paper specifies the test and fixes its inputs. Observed transitions come from the March supplement\u2019s within-interview record of current and prior-year occupation, because two independent codings of an unchanged job disagree 48.1 per cent of the time and the disagreements fall on the classification boundaries where similarity is highest, so a linked panel would find similarity predicting moves whether or not anyone moved. A Poisson pseudo-maximum-likelihood gravity model with origin and destination effects identifies the elasticity of flows to similarity from cross-pair variation, holding the wage differential; adding symmetric pair effects absorbs similarity and identifies the wage differential from the direction of traffic alone, which separates what a score can explain, the level of two-way movement between neighbours, from where that movement nets to. A binned version of the model reads off where flows break and places the breaks against the three thresholds in use. Licensing enters as a reading of the residual map rather than as a regressor. The estimation code is committed and waits on the bilateral matrix.',
    ...
    shortage: {
      ...
      plain: 'Headline shortage figures subtract the supply for an occupation from the demand for that same occupation, as if every opening had to be filled by someone newly trained for it. The occupational flow account shows that in every occupation it covers, more people arrive from other occupations than from education, by a median factor of 19. For five healthcare occupations in North Carolina, arrivals from other occupations at the observed rate equal 49 to 158 percent of the openings the state projects each year, so the channel the standard calculation leaves out is as large as the openings themselves. The paper writes down the correction tier by tier of skill adjacency, with every term measured and capped by history, and infers licensing barriers from the occupation pairs that no one crosses.',
      abs: 'Occupational shortage estimates subtract the supply of an occupation from the demand for that same occupation, which assumes zero substitution, and the supply side of the subtraction is ordinarily the people newly trained for it. The occupational flow account measures where an occupation\u2019s people come from, and arrivals from another occupation exceed arrivals from education in every one of the 172 occupations it covers. This paper measures the first term of the correction and specifies the rest. For registered nurses, nurse practitioners, licensed practical nurses, nursing assistants and medical assistants in North Carolina, arrivals from another occupation at the observed national rate equal 49.2, 157.8, 123.7, 91.9 and 98.9 per cent of the annual openings the state projection expects, with standard errors from the survey\u2019s replicate weights; the arrivals are gross and the projection\u2019s replacement figure already nets part of them out, so the reading is the scale of the omitted channel rather than a cut to the headline. The correction subtracts, tier by tier of skill adjacency, the observed transition rate into the occupation times the slack in the adjacent one, capped at the largest inflow from that tier on record, with adjacency from a composite similarity over 798,342 occupation pairs and willingness from the direction of the wage differential. Licensing is dropped as an input and recovered as a reading: pairs that similarity places close and the account shows nobody crossing are blocked whatever the reason, and whether they concentrate in licensed occupations is tested on the whole set. The distribution of overstatement and its readings are fixed before the bilateral matrix is published.',
