...
(function () {
  ...
  var N = {
    ...
    skillsdna: {
      ...
      plain: 'A labour market can be described three ways at once: by what each occupation is paid, by the tasks each occupation performs, and by the skills those tasks use. The three descriptions have to add up to the same wage bill, and they differ in what they can support. Wages are observed once per occupation, so a description with more tasks than occupations can never be wrong, and a description with fewer skills than occupations can. The teams that have scored how exposed work is to AI each scored one of the three levels, and this paper places each score at the level it was collected, checks whether it survives the trip to the others, and finds the capability measures are largely one measure under three names. Realised wages and employment show nothing through 2025, but every break in those series comes with the pandemic rather than the technology, so the annual data cannot say whether nothing has happened or whether it has not yet reached the stock. The test that can, on the tasks employers ask for in job postings month by month, is specified in the paper and waits on the data.',
      abs: 'Measures of exposure to generative artificial intelligence are collected at three levels of the labour market and compared as though they described one object. This paper writes the labour market as one wage bill at three levels, occupations, tasks and skills, and counts what each can identify: 829 observed wages against an identity, against 17,383 task prices that can never be contradicted, against 25 skill prices that can. Freund and Mann (2026) build the same nesting in general equilibrium and project that the return to social skills rises and the return to analytical skills falls, without testing the projection against realised prices; this paper supplies the empirical counterpart. Each published measure enters at its native layer. Carrying the Eloundou task ratings down to abilities reproduces the Felten ability scores at 0.667 for reading comprehension and 0.546 for language modelling against 0.172 for image recognition, so the propagation discriminates among AI applications correctly. The Srinivasan automation score is the Eloundou construction re-run on a later release, so their 0.833 correlation shows reproducibility, and augmentation is a Herfindahl index over the same classification, explained to 77 percent by a quadratic in automation. Twenty-five named skill groups explain occupational wages at a cross-validated 0.648, price changes are recoverable and levels are not, and inference is built by relabelling year transitions. Within occupations a task’s rated exposure predicts the automation share of its observed use, for the machine rating only. The test of the projection has two halves. On a balanced panel of 693 occupations from 2012 to 2025, no skill group moved by more than 1.51 times its own volatility after 2022, and every deviation from the pre-2020 path begins before language models were released, so the stock returns a null it cannot interpret. The flow half, on the task composition of 155 million job postings at monthly resolution, is specified with its groups, pre-period and readings fixed, and is in progress.',
